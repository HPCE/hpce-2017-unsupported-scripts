...
static void testUniformRandom(const fourier_transform *pTransform, size_t n)
{
	...
	for(unsigned i=0;i<n;i++){
		verifyClose(output[i], input[i], 1e-8, " Non reversible transform.");
	}
}
	...
	try{
		...
		if(!transform->is_quadratic()){
			for(unsigned i=0;i<10;i++){
				testUniformRandom(transform.get(), randomUniformInt(100000)+100);
			}
		}
